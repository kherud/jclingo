...
public class CspGuard extends Ast {

    public CspGuard(Pointer ast) {
        super(ast);
    }
    
    public CspGuard(int comparison, Ast term) {
    ...
    private static Pointer create(int comparison, Ast term) {
        PointerByReference pointerByReference = new PointerByReference();
        Clingo.check(Clingo.INSTANCE.clingo_ast_build(AstType.CSP_GUARD.ordinal(), pointerByReference, comparison, term.getPointer()));
